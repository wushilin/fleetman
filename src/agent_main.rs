...
async fn run_agent_cycle(
    ...
    if cells.is_empty() {
        info!(
            "ℹ️  No cells found for node {} (bucket: {}, prefix: cells/{}/))",
    ...
    for c in observed {
        ...
        match c.profile.to_lowercase().as_str() {
            "deploy" => {
                if let Some(deploy_cfg) = &config.deploy {
                    ...
                    if target_dir.exists() {
                        ...
                        info!(
                            "🧹 Cell removed: moving deploy folder {} -> backup/{}",
                            target_dir.display(),
                            backup_name
                        );
            ...
            "processmaster" => {
                ...
                info!("🧹 Cell removed: pmctl stop {} (best-effort)", app_name);
                ...
                info!("🧹 Cell removed: pmctl update");
                client.update().ok();
            }
            "systemd" => {
                ...
                info!("🧹 Cell removed: systemctl stop {} (best-effort)", service_name);
        ...
        Err(e) => {
            ...
            let (final_version, final_error) = if let Some(prev_version) = previous_version {
                if !config.global.rollback_on_failure {
                    warn!(
                        "🧷 Rollback disabled (global.rollback_on_failure=false). Leaving failed deployment in place for investigation."
                    );
...
async fn deploy_with_systemd(
    ...
    info!("5️⃣ Installing systemd service from app.service");
    ...
    info!("7️⃣ Enabling service: {}", service_name);
...
async fn deploy_with_processmaster(
    ...
    info!("0️⃣ pmctl stop {} (best-effort)", app_name);
    if let Err(e) = client.stop(&app_name) {
        warn!("pmctl stop {} failed (continuing): {}", app_name, e);
    }

    info!("1️⃣ Downloading artifacts to {}", service_dir.display());
    ...
    // pmctl update + start
    info!("2️⃣ pmctl update");
    client.update()?;

    info!("3️⃣ pmctl start {}", app_name);
    client.start(&app_name)?;

    // Give ProcessMaster a short grace window to transition to running.
    tokio::time::sleep(Duration::from_secs(5)).await;
    info!("4️⃣ Verifying processmaster status (json)");
...
async fn rollback_processmaster(
    ...
    info!("🔙 Rollback: pmctl stop {} (best-effort)", app_name);
    if let Err(e) = client.stop(&app_name) {
        warn!("pmctl stop {} failed (continuing): {}", app_name, e);
    }
    ...
    client.update()?;
    info!("🔙 Rollback: pmctl start {}", app_name);
    client.start(&app_name)?;
...
async fn rollback_systemd(
    ...
    info!("🔙 Rollback: Enabling service");
    let output = Command::new(&systemd_config.systemctl_path())
        .args(&["enable", &service_name])
        .output()?;
    if !output.status.success() {
    ...
    info!("🔙 Rollback: Verifying service (waiting 5s)");
    tokio::time::sleep(Duration::from_secs(5)).await;
    verify_systemd_running(systemd_config, &service_name)?;
